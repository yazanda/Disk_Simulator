...
The program is a simulation of processor approaches to the Hard Disk, done using C++, contains writing to and reading from a file that describes the disk.
The disk works with index allocation method to save the data. Receives an action by the user gives him ability to choose what to do (create a new file, write\read, close\open\delete file).
...
1.fsFile - a class that contains information about the file (file size, index block, number of blocks in use and file id).
...
3.fsDisk – a class that contains information about the disk (pointer to the disk file and other parameters and data structures).
...
2.fsFormat - formats the disk (initializes all the data structures of the disk and clears the disk).
3.CreateFile - adds a new file to the disk's database, by initialize a new file descriptor and fsFile.
             - returns the new file's id or -1 if failed.
...
5.CLoseFile - closes the file by remove it from the OpenFileDescriptors map.
            - returns the closed file's id or -1 if failed.
6.WriteToFile - writes to a given fd of a file, checks the maximum file size, disk size, creating new blocks to write and more.
              - returns the number of chars that had been writen or -1 if failed to write all.
7.DelFile - deletes a file by a given file name by remove it from the database of the disk and deletes all it's data on the disk.
          - returns 1 if success or -1 if failed.
8.ReadFromFile - reads a given length of chars from a given file fd on the disk.
               - returns the number chars that had been read or -1 if failed.
...
4.isOpened - checks if a given fd of a file is existed in the openFileDescriptors.
5.isExists - checks if a given file name is exists in the MainDir vector.
6.deleteBlock -  deletes a given block number by writing '\0' in.
